...
namespace Microsoft.MixedReality.Toolkit.Input
{
    ...
    public class MousePointer : BaseControllerPointer, IMixedRealityMousePointer
    {
        ...
        /// <inheritdoc />
        /// MousePointer use Rotate to move cursor 
        /// so its Position always be Vector3.zero
        /// it cause bug with pointer.Position do things with BoundingBox Transforming .
        /// override Position in MousePointer  with BaseCursor.GameObjectReference.transform.position and it should work
        public override Vector3 Position => raycastOrigin != null ? raycastOrigin.position :
           RayStep.GetPointByDistance(Rays, Vector3.Distance(this.transform.position,BaseCursor.GameObjectReference.transform.position));
            //CameraCache.Main.ScreenToWorldPoint(BaseCursor.GameObjectReference.transform.position);
        ...
        public override void OnPreSceneQuery()
        {
            ...
            Ray ray = new Ray(Vector3.zero, Rotation * Vector3.forward);
            Rays[0].CopyRay(ray, PointerExtent);
            Debug.DrawLine(Position, Position+Vector3.forward,Color.red);

            if (MixedRealityRaycaster.DebugEnabled)
            {
                Debug.DrawRay(ray.origin, ray.direction * PointerExtent, Color.cyan);
            }
        }
