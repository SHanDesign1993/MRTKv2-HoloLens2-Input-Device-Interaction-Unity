...
namespace Microsoft.MixedReality.Toolkit.Input
{
    ...
    public class CustomInputSimulationService : BaseInputDeviceManager
    {
        ...
        public CustomSimulatedHandDataProvider handDataProvider = null;

        /// <summary>
        /// Dictionary to capture all active hands detected
        /// </summary>
        public readonly Dictionary<Handedness, SimulatedHand> trackedHands = new Dictionary<Handedness, SimulatedHand>();
        ...
        public override void Update()
        {
            ...
            switch (profile.HandSimulationMode)
            {
                ...
                case HandSimulationMode.Articulated:
                    ...
                    handDataProvider.Update(IsAvailable);
                    break;
        ...
        public void GetHandTransform(Handedness handedness,out Transform transform)
        {
            BaseHandVisualizer hand = null;
            if (handedness == Handedness.Both)
            {
                hand = trackedHands[Handedness.Right].Visualizer as BaseHandVisualizer;
                hand.TryGetJointTransform(TrackedHandJoint.Palm, out transform);
            }
            else
            {
                hand = trackedHands[handedness].Visualizer as BaseHandVisualizer;
                hand.TryGetJointTransform(TrackedHandJoint.Palm, out transform);
            }
        }
        ...
        private void EnableHandSimulation()
        {
            ...
            {
                handDataProvider = new CustomSimulatedHandDataProvider(GetInputSimulationProfile());
